...
async def get_vars(_, message: Message):
    try:
        await app.send_message(
            ...
            text=f"""<u>**{BOT_NAME} Dəyişənləri konfiqurasiya edin :**</u>
...
**sᴇssɪᴏɴ :** `{config.SESSION}`""",
        ...
        return await message.reply_text("» Konfiqurasiya dəyişənlərini göndərmək alınmadı.")
    if message.chat.type != ChatType.PRIVATE:
        await message.reply_text(
            "» Zəhmət olmasa pm-inizi yoxlayın, mən konfiqurasiya dəyişənlərini ora göndərmişəm."
        )
